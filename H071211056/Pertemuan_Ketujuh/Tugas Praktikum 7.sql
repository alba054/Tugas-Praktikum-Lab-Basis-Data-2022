...
-- nomor 3
SELECT ProductName,productScale FROM products as p1 WHERE productName IN ( SELECT productName from products where productName like "%Ford%");
...
(SELECT max(`Total Order`) FROM (SELECT SUM(od.priceEach*od.quantityOrdered) AS "Total Order" from orderdetails GROUP BY orderNumber) as a);
