...
-- nomor 1
-- tidak menggunakan subquery
SELECT c.customerNumber,
MAX(p.amount) AS 'Pembayaran Terbesar', MIN(p.amount)AS 'Pembayaran Terkecil'
FROM customers AS c
JOIN payments AS p
...
o.orderNumber AS 'Nomor Order', SUM(od.priceEach * od.quantityOrdered) AS 'Biaya Pesanan'
FROM customers AS c
...
GROUP BY `Nama Pelanggan`, `Nama Karyawan`, `Nomor Order`
